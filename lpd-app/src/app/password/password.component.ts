...
export class PasswordComponent implements OnInit {
	...
	setPassword(evt) {
		...
		else this.userService.setPassword(this.token, this.passwordForm.value.password).subscribe(json => {
			console.log(json);
	...
	matchPasswords(c: AbstractControl): { 'invalid': boolean } {
		if (c.get('password').value !== c.get('confirm').value || !c.get('password').value.length) return { 'invalid': true };
		return null;
	}
