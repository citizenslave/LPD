...
export class LoginComponent implements OnInit {
	...
	loginUser(evt, register: boolean = false) {
		evt.stopPropagation();
		...
		if (register) {
			...
			this.isLoginLoading = true;
			this.userService.findUser(this.loginForm.value.username).subscribe(responseStatus => {
				this.publicKey = true;
				this.isLoginLoading = false;
			...
			this.isLoginLoading = true;
			this.userService.loginUser(this.loginForm.value.username, this.loginForm.value.password).subscribe(json => {
				this.loggedIn = true;
	...
	openUser(evt) {
		...
		(<any[]> Object.values(permissions)).forEach(permission => {
			if ((user.permissions & Number(permission.mask)) || (user.permissions & Number(permissions['SYSADMIN'].mask)))
	...
	changePermission(username, mask) {
