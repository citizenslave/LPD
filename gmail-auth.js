...
module.exports = class GMailer {
	...
	constructor() {
		fs.readFile(__dirname+'/credentials.json', (error, credentials) => {
			const { client_secret, client_id, redirect_uris } = JSON.parse(credentials).installed;
			const oAuth2Client = new google.auth.OAuth2(client_id, client_secret, redirect_uris[0]);
			fs.readFile(__dirname+'/token.json', (error, token) => {
				if (error) {
					...
					rl.question('CODE: ', (code) => {
						...
						oAuth2Client.getToken(code, (error, token) => {
							...
							fs.writeFile('token.json', JSON.stringify(token), (e) => {
								...
								this.sendMail('mcvay.will@gmail.com', 'TEST', '<em>SUCCESS</em>').then(console.log).catch(console.error);
							});
						});
					});
				} else {
